...
export const getInstructorDashboard = async (req: Request, res: Response) => {
  try {
    ...
    enrollAgg.forEach(e => { perCourseStats[e._id.toString()] = e; });
...
export const getInstructorCourses = async (req: Request, res: Response) => {
  try {
    ...
    const [courses, total] = await Promise.all([
      Course.find({ instructor: instructorId }).sort({ createdAt: -1 }).skip(skip).limit(limit).lean(),
    ...
    const enrollCounts = await Enrollment.aggregate([
      { $match: { course: { $in: courseIds } } },
      { $group: { _id: "$course", studentsCount: { $sum: 1 }, avgComplete: { $avg: "$percentComplete" } } },
    ]);
...
export const getCourseById = async (req: Request, res: Response) => {
  try {
    ...
    const course = await Course.findOne({ _id: id, instructor: instructorId }).lean();
...
export const updateCourse = async (req: Request, res: Response) => {
  try {
    ...
    const course = await Course.findOneAndUpdate({ _id: id, instructor: instructorId }, req.body, { new: true });
    if (!course) return res.status(404).json({ message: "Course not found or you are not the instructor" });
...
export const deleteCourse = async (req: Request, res: Response) => {
  try {
    ...
    return res.json({ message: "Course deleted successfully", courseId: id });
  } catch (err: any) {
    console.error(err);
    return res.status(500).json({ message: "Server error", error: err.message });
  }
...
export const publishCourse = async (req: Request, res: Response) => {
  try {
    ...
    return res.json({ message: `Course ${course.published ? "published" : "unpublished"}`, courseId: id, published: course.published });
  } catch (err: any) {
    console.error(err);
    return res.status(500).json({ message: "Server error", error: err.message });
  }
