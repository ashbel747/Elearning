...
import { fetchCourses, deleteCourse as apiDeleteCourse } from "../../api/instructorApi";
...
const Courses: React.FC = () => {
  ...
  const loadCourses = async () => {
    try {
      ...
      else if (Array.isArray((data as any)?.courses)) setCourses((data as any).courses);
      else setCourses([]);
    } catch (err) {
      console.error(err);
  ...
  if (loading) return <p className="p-6 text-center text-gray-600">Loading courses...</p>;
  if (error)
    return (
      <div className="p-6 text-center text-red-600">
        {error}{" "}
        <button onClick={loadCourses} className="underline text-indigo-600 hover:text-indigo-800">
          Retry
        </button>
      </div>
    );
              ...
              {courses.map(course => (
                <tr key={course._id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-2 border-b">
                    {course.published ? (
                      <span className="px-2 py-1 text-xs font-medium bg-green-100 text-green-700 rounded">Yes</span>
                    ) : (
                      <span className="px-2 py-1 text-xs font-medium bg-yellow-100 text-yellow-700 rounded">No</span>
                    )}
                  </td>
