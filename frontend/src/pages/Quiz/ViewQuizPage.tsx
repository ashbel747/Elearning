...
const ViewQuizPage: React.FC = () => {
  ...
  useEffect(() => {
    if (!lessonId || !token) {
      console.log("Missing lessonId or token:", { lessonId, token: token ? "present" : "missing" });
      return;
    }
    ...
    const fetchQuiz = async () => {
      try {
        ...
        console.log("Setting auth token and fetching quiz for lesson:", lessonId);
        console.log("Token present:", !!token);
                setAuthToken(token);
        
        const data = await getQuizByLesson(lessonId) as QuizWithMetadata;
        ...
        if (err && typeof err === 'object' && 'response' in err) {
          ...
          if (status === 401) {
            ...
            errorMessage = serverMessage || "You don't have permission to view this quiz.";
          } else if (status === 404) {
            errorMessage = serverMessage || "Quiz not found for this lesson.";
          } else {
    ...
    <div className="max-w-4xl mx-auto p-6">
      ...
      <div className="mb-8 bg-blue-50 border border-blue-200 rounded-lg p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
          <div>
            <span className="font-medium text-gray-600">Lesson ID:</span>
          ...
          <div>
            <span className="font-medium text-gray-600">Question Types:</span>
            <p className="text-gray-900">
              {[...new Set(quiz.questions?.map(q => q.type.replace('-', ' ')) || [])].join(", ")}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-6">
        {quiz.questions && quiz.questions.map((question, index) => (
          <div key={question._id} className="bg-white border border-gray-200 rounded-lg p-6 shadow-sm">
            {/* Question Header */}
            ...
            {question.type === "true-false" && (
              <div className="ml-4">
                <p className="text-sm font-medium text-gray-600 mb-2">Format: True/False Question</p>
                <div className="space-y-1">
                  <div className="flex items-center space-x-2">
                    <span className={`w-2 h-2 rounded-full ${
                      question.correctAnswer === 'true' ? 'bg-green-500' : 'bg-gray-300'
                    }`}></span>
                    <span className={question.correctAnswer === 'true' ? 'font-medium text-green-700' : 'text-gray-700'}>
                      True {question.correctAnswer === 'true' && '✓ Correct'}
                    </span>
                  </div>
                  <div className="flex items-center space-x-2">
                    <span className={`w-2 h-2 rounded-full ${
                      question.correctAnswer === 'false' ? 'bg-green-500' : 'bg-gray-300'
                    }`}></span>
                    <span className={question.correctAnswer === 'false' ? 'font-medium text-green-700' : 'text-gray-700'}>
            ...
            {question.type === "short-answer" && (
              <div className="ml-4">
                <p className="text-sm font-medium text-gray-600 mb-2">Format: Short Answer (Text Input)</p>
      ...
      <div className="mt-8 bg-gray-50 border border-gray-200 rounded-lg p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
          <div>
            <p><strong>Quiz ID:</strong> {quiz._id}</p>
            <p><strong>Created:</strong> {formatCreatedDate(quiz)}</p>
          </div>
          <div>
            <p><strong>Total Questions:</strong> {quiz.questions?.length || 0}</p>
            <p><strong>Status:</strong> <span className="text-green-600">Published</span></p>
          </div>
        </div>
      </div>
    </div>
