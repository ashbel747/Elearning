...
const QuizPage: React.FC = () => {
  ...
  return (
    <div className="max-w-3xl mx-auto p-4">
      {/* Timer Display */}
      <div className={`fixed top-4 right-4 p-3 border rounded-lg shadow-lg ${getTimerBgColor()} transition-colors duration-300`}>
      ...
      {timeLeft <= 30 && timeLeft > 0 && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-lg text-center">
            <h3 className="text-xl font-bold text-red-600 mb-4">Time Almost Up!</h3>
            <p className="mb-4">Only {timeLeft} seconds remaining. Submit your answers soon!</p>
            <button
      ...
      <div className="mb-6">
        <div className="flex justify-between items-center mb-2">
          <span className="text-sm text-gray-600">
            Answered: {Object.keys(state.answers).length}/{state.currentQuiz.questions.length}
          </span>
          <span className="text-sm text-gray-600">{formatTime(timeLeft)} remaining</span>
        </div>
        <div className="w-full bg-gray-200 rounded-full h-2">
          <div 
            className="bg-blue-500 h-2 rounded-full transition-all duration-300"
            style={{ width: `${(Object.keys(state.answers).length / state.currentQuiz.questions.length) * 100}%` }}
          ></div>
        </div>
      </div>
      ...
      {Object.keys(state.answers).length < state.currentQuiz.questions.length && (
        <div className="mt-4 p-3 bg-yellow-50 border border-yellow-300 rounded">
          <p className="text-yellow-700">
            ⚠️ You have {state.currentQuiz.questions.length - Object.keys(state.answers).length} unanswered questions.
          </p>
        </div>
      )}
    </div>
