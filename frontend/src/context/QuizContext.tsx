...
import { getQuizByLesson, submitQuizAttempt, createQuiz, setAuthToken } from '../services/quizApi';
...
type QuizAction =
  ...
  | { type: 'UPDATE_QUESTION_IN_DRAFT'; payload: { index: number; question: Partial<Question> } }
  | { type: 'REMOVE_QUESTION_FROM_DRAFT'; payload: number }
  | { type: 'SET_ANSWER'; payload: { questionId: string; answer: string } }
  | { type: 'SET_QUIZ_RESULT'; payload: { score: number; submitted: boolean } }
...
export const QuizProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const updateQuestion = useCallback((index: number, updates: Partial<Question>) => {
    dispatch({ type: 'UPDATE_QUESTION_IN_DRAFT', payload: { index, question: updates } });
  }, []);
  ...
  const saveQuiz = useCallback(async (token: string) => {
    if (!state.quizDraft) {
      dispatch({ type: 'SET_ERROR', payload: 'No quiz draft to save' });
      return;
    }
    ...
    try {
      ...
      if (error && typeof error === 'object' && 'response' in error) {
        ...
        if (status === 401) {
          ...
          errorMessage = serverMessage || 'You don\'t have permission to create quizzes.';
        } else if (status === 400) {
          errorMessage = serverMessage || 'Invalid quiz data. Please check your inputs.';
        } else {
          errorMessage = serverMessage || axiosError.message || errorMessage;
        }
      } else if (error instanceof Error) {
  ...
  const submitQuiz = useCallback(async (userId: string, token: string) => {
    ...
    try {
      ...
      const attemptPayload = {
        ...
        userId,
        answers: Object.entries(state.answers).map(([qId, answer]) => ({
          questionId: qId,
          answer,
        })),
      };
